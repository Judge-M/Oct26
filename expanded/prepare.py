...
def fat16(path,files,day):
    ...
    for i,(name,content,deleted) in enumerate(files):
        ...
        if not 1980 <= stamp.year <= 2107:
            raise ValueError('Date outside FAT range')
        struct.pack_into('<HHHI',image,entry+22,0x4820,((stamp.year-1980)<<9)|(stamp.month<<5)|stamp.day,cluster,len(content))
        count=max(1,(len(content)+511)//512)
        if i >= 512 or (data_sector+cluster-2+count) > sectors:
            raise ValueError('FAT16 fixture capacity exceeded')
...
def dns_capture(path,rows):
    raw=bytearray(struct.pack('<IHHIIII',0xa1b2c3d4,2,4,0,0,65535,1))
    for row in rows:
        name,client,address=row['query'],row['client'],row['answer']
        ...
        for response in (False,True):
            ...
            if response:payload+=b'\xc0\x0c'+struct.pack('!HHIH',1,1,int(row['ttl']),4)+socket.inet_aton(address)
            ...
            stamp=int(datetime.fromisoformat(row['time']).timestamp())
...
def build(output,config=None):
    config=config or json.loads((REPO/'expanded/config.json').read_text(encoding='utf-8'))
    day=incident_day(config['exercise_date'])
    baseline=json.loads((REPO/'config.json').read_text(encoding='utf-8'))
    baseline['exercise_date']=day
    output=Path(output)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write(comparison,comparison.read_text(encoding='utf-8')+'superseded_utc,not_applicable,09:10:00\n')
        ...
        with closing(sqlite3.connect(public/'endpoint/browser.sqlite')) as con:
            con.row_factory=sqlite3.Row
            rows=[dict(row) for row in con.execute('SELECT * FROM downloads')]
        generate.write_csv(public/'browser/downloads.csv',rows)
    dns_capture(public/'network/dns.pcap',read_csv(public/'network/dns.csv'))
    syslog_capture(public/'network/sensor.pcap',read_csv(public/'network/proxy.csv'))
    hunt=[json.loads(line) for line in (public/'hunting/siem.jsonl').read_text(encoding='utf-8').splitlines()]
    ...
    fixtures={
      'windows-process':{'host':'WS-17','device_time':day+'T09:00:00Z','utc':day+'T08:58:00Z','pid':4240,'parent':'browser.exe'},
      'windows-task':{'host':'WS-17','device_time':day+'T09:10:00Z','utc':day+'T09:08:00Z','task':'BriefSync','executable':'brief-viewer.exe'},
    ...
    for path in sorted(public.rglob('*.csv')):
        for row in read_csv(path):
            row=dict(row,type='coverage' if path.name=='coverage.csv' else 'historical')
            telemetry.append(telemetry_record(row,path.relative_to(public).as_posix()))
    ...
    for i in range(400):
        ...
        telemetry.append(dict(timestamp=f'{day}T{8+minute//60:02}:{minute%60:02}:{rng.randrange(60):02}Z',
    ...
    telemetry.sort(key=lambda r:(r.get('timestamp',''),r['record_id']))
    ...
    release_manifest={'schema':1,'exercise_date':day,'tickets':{}}
    for folder in sorted((vault/'releases').iterdir()):
        release_manifest['tickets'][folder.name]={p.relative_to(folder).as_posix():sha256(p)
...
if __name__=='__main__':
    ...
    p.add_argument('--config',type=Path,default=REPO/'expanded/config.json')
    a=p.parse_args();print(build(a.output,json.loads(a.config.read_text(encoding='utf-8'))))
