...
class Journal:
    ...
    def lock(self, holder, timeout=30, poll=0.05, stale=LOCK_STALE_SECONDS):
        ...
        try:
            with process_lock(self.path.with_suffix('.lock'), timeout, poll):
                yield
        ...
        with self.journal.lock(self.actor, timeout=timeout):
            for step in plan:
                ...
                if current and current['state'] == 'verified':
                    if not verify_resource(self.provider, step['kind'], step['name'], current['resource_id']):
                        raise DeploymentError('verified resource is missing or changed: ' + step['name'])
                    continue
                ...
                try:
                    ...
                    raise
            # Infrastructure receipts alone cannot certify application pause/readiness.
            self.journal.set_state('RESOURCES_VERIFIED')
            return self.journal.state

    def stop(self, timeout=30):
        with self.journal.lock(self.actor, timeout=timeout):
            for resource in self.journal.resources():
                if self.provider.stop(resource['kind'], resource['name'], resource['provider_id']) is not True:
                    raise DeploymentError('resource stop was not verified: ' + resource['name'])
