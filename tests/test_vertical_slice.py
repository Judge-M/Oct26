...
class VerticalSliceTests(unittest.TestCase):
    ...
    def test_main_reports_live_blocker(self):
        with patch.dict(os.environ, ENV):
            with patch('sys.argv', ['vertical_slice', '--state', 's', '--config', 'c', '--operator', 'op']):
                with self.assertRaisesRegex(SliceError, 'rehearsal host'):
                    main()
