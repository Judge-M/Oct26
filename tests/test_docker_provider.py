...
class LifecycleTests(unittest.TestCase):
    ...
    def test_create_checks_image_then_creates_service(self):
        ...
        self.assertEqual(argv, ['docker', 'compose', '--project-name', 'silent-ridge-desktops', '-f', str(COMPOSE), 'create', 'desktop-team01'])
    ...
    def test_start_and_stop_use_service_names(self):
        self.assertEqual(self.provider.start('team02'),
                         ['docker', 'compose', '--project-name', 'silent-ridge-desktops', '-f', str(COMPOSE), 'start', 'desktop-team02'])
        self.assertEqual(self.provider.stop('desktop-team02'),
                         ['docker', 'compose', '--project-name', 'silent-ridge-desktops', '-f', str(COMPOSE), 'stop', 'desktop-team02'])
...
class ProbeTests(unittest.TestCase):
    def _provider(self, entry, ps='container-id'):
        runner = FakeRunner()
        runner.when(lambda argv: argv[1] == 'compose' and 'ps' in argv, ps)
...
class InventoryTests(unittest.TestCase):
    def test_inventory_reports_every_container_sorted(self):
        ...
        runner = FakeRunner()
        runner.when(lambda argv: argv[1] == 'compose' and 'ps' in argv, 'id1\nid2')
