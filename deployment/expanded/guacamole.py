...
def _desktops(config):
    ...
    for id,d in desktops.items():
        if d.get('container'):
            d['address'] = container_address(id)
...
def reconcile(config,credentials):
    ...
    sql=['BEGIN;', 'LOCK TABLE guacamole_connection IN SHARE ROW EXCLUSIVE MODE;']
    for id,d in desktops.items():
        ...
        sql.append('INSERT INTO guacamole_connection(connection_name,protocol,max_connections,max_connections_per_user) SELECT '+
                   ','.join([quote(id),quote('vnc'),str(maximum),str(maximum)])+
                   ' WHERE NOT EXISTS (SELECT 1 FROM guacamole_connection WHERE connection_name='+quote(id)+' AND parent_id IS NULL);')
        sql.append('UPDATE guacamole_connection SET protocol=\'vnc\',max_connections='+str(maximum)+
                   ',max_connections_per_user='+str(maximum)+' WHERE connection_name='+quote(id)+' AND parent_id IS NULL;')
